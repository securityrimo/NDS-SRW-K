...
슈퍼로봇대전 K 타이틀 로고 「슈퍼로봇대전 K」를 직접 다듬을 수 있는 PNG 캔버스입니다.
...
| 파일 | 설명 |
|---|---|
| `title_logo_canvas.png` (256×224) | **편집 대상**. 화면과 1:1 — 픽셀 (x,y) = 게임 화면 좌표 |
| `title_logo_canvas_x4.png` (1024×896) | 같은 그림 4배 (편집 편의용, import가 자동 축소) |
| `title_logo_guide.png` | 편집 가능 영역(밝은 회색 = 스프라이트가 덮는 곳) |

둘 중 **하나만** 편집해서 저장하세요(캔버스 또는 x4). 현재 한글 버전이 들어 있으니 처음부터가 아니라 다듬으면 됩니다.
...
## 추출 / 되돌리기(주입)
> 전제: 타이틀 화면의 DeSmuME 세이브스테이트에서 뽑은 `_sstate.bin`(OAM) · `_vram.bin`(OBJ 타일) · `_pal.bin`(팔레트), 그리고 현재 타일 `_fg_new.bin`이 작업 폴더에 있어야 합니다(저작권상 저장소에는 미포함).

```bash
# 추출: 현재 로고 -> PNG 캔버스
python tools/_export_png.py

# 편집한 PNG -> 타일로 되돌리기 -> ROM
